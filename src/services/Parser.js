...
const TOKEN_TYPES = {
    ACCIDENT_ID: 'ACCIDENT_ID',             // Identificador del accidente
    DATE: 'DATE',                           // Fecha del accidente
    TIME: 'TIME',                           // Hora del accidente
    LOCATION: 'LOCATION',                   // Ubicacion del accidente
    VEHICLE_TYPE: 'VEHICLE_TYPE',           // Tipo de vehiculo involucrado
    SEVERITY: 'SEVERITY',                   // Gravedad del accidente
    CASUALTIES: 'CASUALTIES',               // Numero de victimas
    WEATHER_CONDITION: 'WEATHER_CONDITION', // Condicion climática
    ROAD_CONDITION: 'ROAD_CONDITION',       // Condicion de la vía
    DRIVER_AGE: 'DRIVER_AGE',               // Edad del conductor
    ALCOHOL_INVOLVED: 'ALCOHOL_INVOLVED',   // Alcohol involucrado
    SPEEDING_INVOLVED: 'SPEEDING_INVOLVED'  // Exceso de velocidad
  };
  ...
  class Parser {
    ...
    performLexicalAnalysis() {
      ...
      this.csvData.forEach((row, rowIndex) => {
        ...
        Object.entries(row).forEach(([key, value]) => {
          ...
          const tokenType = TOKEN_TYPES[normalizedKey] || 'UNKNOWN';
    ...
    generateReport(semanticResult) {
      ...
      return {
        ...
        summary: {
          'Total de accidentes analizados': this.csvData.length,
          'Accidentes válidos': validRows.length,
          'Total de víctimas': totalCasualties,
